defmodule Relay.Registry.Locations do
  ...
  def handle_call({:register_location, location, pid}, _from, _state) do
    :ets.insert(@table, { location, pid })
  ...
  def register_location(location = %Location.Irc{}, dispatch_pid, location_pid) when is_pid(dispatch_pid) and (is_pid(location_pid) or is_nil(location_pid)) do
    GenServer.call(__MODULE__, { :register_location, location, dispatch_pid })
  end

  def register_location(location = %Location.Slack{}, dispatch_pid, location_pid) when is_pid(dispatch_pid) and (is_pid(location_pid) or is_nil(location_pid)) do
    GenServer.call(__MODULE__, { :register_location, location, dispatch_pid })
  end
